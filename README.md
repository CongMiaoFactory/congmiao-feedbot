...
- Bilibili 动态支持纯文字、图片/Opus、转发、投稿视频、番剧、专栏、音频和直播卡片；完整 Opus 正文不可用时会自动回退到动态摘要。
- 动态图片会携带 Bilibili Referer 下载并写入稳定媒体缓存键，支持 `+pN` 选择单图和回复后使用 `/file` 获取原图。
- Opus 详情会按需补充 `buvid3` 设备 Cookie；风控或登录状态不足时不影响普通动态和封面降级。
